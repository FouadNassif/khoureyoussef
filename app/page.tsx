...
export default function Page() {
  ...
  const newsItemsRef = useRef<Array<HTMLDivElement | null>>([]);
  ...
  const resources = (i18n.getResourceBundle(i18n.language, "translation") || {}) as Record<string, any>;

  // Get featured miracle
  const featuredMiracle = useMemo<Miracle | undefined>(() => {
    const miracles = Object.values(resources).filter(
      (item: any) => item?.type === "Healing" || item?.type === "شفاء" || item?.type === "Guérison"
    ) as Miracle[];
    return miracles.find((m) => m.isFeatured) || miracles[0];
  }, [resources]);
