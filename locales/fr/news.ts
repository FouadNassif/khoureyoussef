export default {
    newsItems: [
        {
            id: 1,
            title: "Comité sur Mgr Youssef Soueif",
            content: "Son Excellence Mgr Youssef Soueif parle du comité formé pour étudier le dossier du Père Youssef Abi Maroun Maatouk. Résidence d'été de l'évêché à Karm Saddé le 1-1-2023",
            date: "2023-01-01",
            video: "/assets/videos/news/Video1.mp4",
            thumbnail: "/assets/thumbnails/Video1.jpg",
            type: "video"
        },
        {
            id: 2,
            title: "Sermon de Son Excellence Mgr Youssef Soueif",
            content: "Extrait du sermon de Son Excellence Mgr Youssef Soueif sur le Père Youssef Abi Maroun Maatouk lors de la fête du Père Youssef le 8-11-2022 dans l'église du Père Youssef, Sereel",
            date: "2022-11-08",
            video: "/assets/videos/news/Video2.mp4",
            thumbnail: "/assets/thumbnails/Video2.jpg",
            type: "video"
        },
        {
            id: 3,
            title: "Fairouz dans l'église du Père Youssef Sereel",
            content: "",
            date: "2010-01-01",
            video: "https://youtu.be/V68VbGRj-QY?si=091trsX0ZeZo-1SJ",
            thumbnail: "/assets/thumbnails/Video3.jpg",
            type: "video"
        },
        {
            id: 4,
            title: "Hymne",
            content: "",
            date: "2024-08-28",
            type: "video",
            video: "/assets/videos/news/Video3.mp4",
            thumbnail: "/assets/thumbnails/Video4.jpg",
        }
    ]
};
